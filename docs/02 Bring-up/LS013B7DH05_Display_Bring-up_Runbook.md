...
| Level translator OE | `PD2` `VLT_LCD` | active-low TXU0104RUTR OE; high disables/high-Z, low enables |
| EXTCOMIN / VCOM | `PC13` `LCD_1HZ` | RTC 1 Hz calibration output |

Safe boot default is `VLT_LCD` high / disabled until `thDisplay` owns the display path. After display initialization, the default hold policy is `VLT_LCD` low / enabled so SPI and `LCD_1HZ` can reach the panel.
...
1. Boot with `VLT_LCD` high and SPI3 idle.
2. Start the display owner and drive `VLT_LCD` low.
...
| --- | --- | --- | --- | --- |
| safe boot | `VLT_LCD` high | translator disabled/high-Z | TBD | open |
| display enable | `VLT_LCD` low | translator enabled | TBD | open |
...
1. Confirm `VLT_LCD` active-low OE behavior: high disables TXU0104RUTR outputs, low enables outputs.
