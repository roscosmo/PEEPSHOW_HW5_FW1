...
| USB device | `USB_OTG_FS` detection path and USBX personality | `PA11` DM, `PA12` DP, `PA9` VBUS | `thStorage` / `thPower` | none in current `.ioc` | VBUS attach for power/detect policy | detached / not exported; USB activity/enumeration gates MSC availability |
