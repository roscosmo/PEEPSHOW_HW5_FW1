...
This note records the intended HW5 clock tree and the current CubeMX baseline.
...
|---|---|---|---|
| SYSCLK / HCLK | MSI 24 MHz baseline | current `.ioc` has SYSCLK/HCLK 24 MHz | validate after STOP resume |
...
| BBB LPTIM1 | current CubeMX LPTIM path via `PB2` `BUZZ` | procedural BBB tone/sweep timing must be bounded | audio owner stops BUZZ output before STOP |
